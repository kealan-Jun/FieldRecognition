...
def install(core):
    ...
    def listing():
        with core['db']() as conn:
            return {'items': [json.loads(r[0]) for r in conn.execute('SELECT document FROM binding_handoffs ORDER BY rowid DESC LIMIT 100')],
                'identity_assurance': 'local_registered_operator_not_authenticated_account'}
    ...
    def scans():
        ...
        return {'items': [{k: doc.get(k) for k in ('scan_id', 'camera_id', 'received_at', 'matches')}
                           for row in rows for doc in [json.loads(row[0])]]}
    ...
    def request(body: HandoffRequest):
        with core['live_scanner'].lock, core['db']() as conn:
            ...
            transfer = {'handoff_id': str(body.request_id), 'instrument_id': old['instrument']['id'],
                ...
                'identity_assurance': 'local_registered_operator_not_authenticated_account'}
            conn.execute('INSERT INTO binding_handoffs VALUES(?,?,?,?)',
    ...
    def transition(hid: uuid.UUID, action: str, body: HandoffDecision):
        if action not in {'release', 'accept', 'cancel'}:
            raise HTTPException(404, '交接动作不存在')
        with core['live_scanner'].lock, core['db']() as conn:
            conn.execute('BEGIN IMMEDIATE')
            transfer = fetch(conn, hid)
            expected_actor = transfer['recipient_operator'] if action == 'accept' else transfer['source_operator']
            expected_camera = transfer['recipient_camera_id'] if action == 'accept' else transfer['source_camera_id']
            ...
            if action == 'release':
                ...
                transfer.update(status='cancelled', cancelled_at=timestamp)
