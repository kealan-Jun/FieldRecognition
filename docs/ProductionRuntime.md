...
默认 `FIELD_AUTH_ENABLED=0`，网页、API 和 Agent 均无需登录。准备脚本只登记使用人唯一 ID、姓名与相机关系，不生成密码；已有账号、绑定、原图和回执保持原样。实验员登记仍用于数据归属，免登录下的操作人是本机登记或请求声明的身份，不代表通过账号认证。
...
已占用仪器不能由另一相机静默接管。接收人需实际扫码申请，原使用人明确交出后，接收人确认；请求跨过北京时间零点即失效，必须使用当天新的二维码证据。相机使用人重新分配前必须结束现有绑定。晚到照片按拍摄时间匹配历史绑定，无法可靠归属时保持待处理，不改写旧归属。
