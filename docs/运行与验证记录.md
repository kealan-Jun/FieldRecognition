# 运行与验证记录

> 本文保留首页改版前的部署说明、接口说明与分阶段验证记录。各节中的测试数量和现场状态对应当时的验证，不代表新增测试结果。项目概览见 [README](../README.md)，完整实现说明见 [技术方案](../技术方案.md)。文中的本地路径和命令仍以项目根目录为基准。

## 本机运行与 NAS 留存验证（2026-09-14）

按使用者最新确认，识别程序继续部署在本机，NAS 只作为现有语音照片源和本项目独立归档目标，不部署 NAS 服务，也无需 NAS 管理员账号。部署方法见 [本机运行与 NAS 留存](本机运行与NAS留存.md)。

- `PROVEN`：103 项自动测试通过，Python 编译、JavaScript 语法检查通过。新增测试覆盖原始字节与哈希、事务回滚、版本保留、失联重试、幂等、坏文件隔离、索引恢复，以及绑定起止与 OCR 结果的照片关联。测试使用临时数据与模型替身，不访问真实 NAS 或调用付费模型。
- `PROVEN`：实际共享目录 `/mnt/realityloop-nas/FieldRecognitionArchive/` 读写成功。11:51（北京时间）核对时，33 条已有业务快照已归档、待同步 0 条；每份回执的内容与 SHA-256、29 个独立图片对象的大小与 SHA-256 全部核对通过。本机原有 29 张图片哈希及业务表记录数量保持不变。此前历史记录中包含旧验证材料，归档总数不等同于真实现场绑定次数。
- `PROVEN`：此前现场仪器 A 的二维码命中帧及 10:47:26 的原接收时间已留存到 NAS。旧版仅有规范化图片，回执标明原始压缩字节未留存；没有由旧图补建新的绑定。后续新照片开始保留原始字节。
- `PROVEN`：本机服务重启后相机在线、自动扫码运行，实验员登记保持徐荣炜，OCR 设备配置为 `gpu:0`。网页“操作记录”显示 NAS 已连接和归档计数。
- `NOT_PROVEN`：真实设备的新扫码自动绑定、语音拍照到真实面板读数完整闭环及物理服务停启验收。使用者选择稍后进行；存储校验与模型替身测试不替代现场准确率验证。

数据库改造前已创建本机 SQLite 备份；核对回执为 `Verification/LocalNasArchive20260914/Receipt.json`，原图哈希清单为同目录 `Before.json`，均不随 Git 分发。NAS 的 `Readme.html` 与 `Index.json` 提供索引，完整版本回执与图片分别保存在 `Receipts/` 和 `Objects/`。

独立的二维码识别、场景登记、设备绑定与真实 PaddleOCR 面板识别系统。复用现有 Agent 的语音拍照文件，只读取 NAS 中当前绑定相机的照片，不修改 VisionCortex 采集或预处理服务。

完整设计与实现说明见 [项目技术方案](../技术方案.md)，包含二维码制作与解码、绑定生命周期、视频协议、NAS 照片监控、OCR、视觉兜底、数据与接口、部署和验证边界。

## 独立项目

项目仓库：[kealan-Jun/FieldRecognition](https://github.com/kealan-Jun/FieldRecognition)。

本机项目根目录为 `/home/x1/Projects/FieldRecognition`，使用独立 Git 仓库和 Python 虚拟环境。历史证据中的旧目录引用保留原样；当前图片接口按 capture_id 从本项目 Data/Images 读取。Python 3.11.15 由用户级 uv 运行时提供，不依赖 VisionCortex 的解释器、代码或配置。

| 内容 | 路径 |
|---|---|
| FastAPI 服务、异步 OCR 与存证 | `app.py` |
| 挂脖设备 GWHP 主码流接收 | `receiver.py` |
| 二维码解码、场景绑定 | `qr_decode.py`、`scene_binding.py` |
| Agent HTTP 工具与客户端 | `agent_tools.py`、`agent_client.py`、`AgentTools.md` |
| 网页和二维码样张 | `static/` |
| 仪器、场景初始登记 | `InstrumentRegistry.json`、`SceneRegistry.json` |
| 依赖、测试、服务模板 | `Requirements.lock.txt`、`tests/`、`deployment/` |
| 本地照片、数据库 | `Data/` |
| 历史验证、标签交付、临时材料 | `Verification/`、`VerificationData/`、`output/`、`tmp/` |

迁移保留全部已有本地材料；运行数据、历史验证原件、模型、虚拟环境和日志均不纳入 Git。下文的 `Verification/` 引用只在本机历史材料中存在，新克隆不会附带这些实拍数据或回执。模型缓存仍使用 PaddleOCR/PaddleX 的用户级默认位置，不复制进代码仓库。

### 新环境安装

以下命令用于新克隆或新环境；本机已迁移环境无需重建。需要已安装 uv，并将终端切换到项目根目录：

```bash
uv python install 3.11.15
uv venv --python 3.11.15 .venv
uv pip install --python .venv/bin/python -r Requirements.lock.txt
```

`Requirements.lock.txt` 是已有环境的完整版本清单，包含运行和测试依赖。模型权重不在仓库内；首次成功绑定后会在后台加载 OCR 模型，缺少缓存时可能下载权重；加载成功后保持在本地识别服务内存中。

### 本地启动与测试

不配置 receiver 时，可使用照片上传和二维码样张。启动命令：

```bash
.venv/bin/python -m uvicorn app:app --host 127.0.0.1 --port 8188
```

若本机用户服务已运行，请直接使用现有页面；同一端口不能再启动第二个实例。运行已有测试：

```bash
env -u FIELD_RECEIVER_URL -u FIELD_CAMERA_SNAPSHOT_URL OMP_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2 .venv/bin/python -m pytest -q tests
```

测试使用临时数据库和样张，不启动 PaddleOCR 模型，不连接真实 receiver。

### 用户服务安装

服务模板为 [deployment/field-recognition-demo.service](../deployment/field-recognition-demo.service)，默认项目位置为 `~/Projects/FieldRecognition`。其他安装位置须调整模板中的工作目录、启动命令和环境文件路径。新安装时：

```bash
cp .env.example .env
# 在 .env 中填写本机 receiver 与 camera 设置；不要提交 .env。
mkdir -p ~/.config/systemd/user
cp deployment/field-recognition-demo.service ~/.config/systemd/user/field-recognition-demo.service
systemctl --user daemon-reload
systemctl --user enable --now field-recognition-demo.service
```

本机已有服务的配置在迁移时保留，只更新项目路径。不要用新安装模板覆盖已有相机配置；修改实际服务的环境配置后执行 `systemctl --user daemon-reload` 和 `systemctl --user restart field-recognition-demo.service`。手动启动的进程不会自动读取 `.env`，需在启动前设置环境变量。

## 使用

打开 http://127.0.0.1:8188/ 。

1. 在“仪器登记”中填写两台仪器的实际名称、型号和所属场景。
2. 填写实验员，点击“连续扫码绑定”。页面显示挂脖实时视频；先对准场景码，再对准仪器码。唯一场景自动进入，唯一且场景匹配的仪器只需一帧成功解码即可自动绑定。多个候选会暂停并要求手动选择。
3. 绑定成功后停止扫码，实时视频继续显示；OCR 模型同时在后台自动加载，页面显示“模型常驻 · 已就绪”后供后续识别复用。重复扫码、重复提交和刷新页面均沿用原绑定，不新增记录。设备采集服务离线或采集会话变化后旧绑定结束，服务恢复后重新点击连续扫码。
4. 对准面板，对现有 Agent 说“拍照”。照片写入该相机的 NAS 语音拍照目录并稳定后，本服务自动对这张照片识别，不重新拍照。也可在网页上传照片、框选并识别。
5. 页面统一显示读数。OCR 请求开始 5 秒后仍无完整数字时，调用阿里云视觉模型兜底；结果仍需核对，不据此推断物理实验动作。
6. 结束绑定或导出 JSON。原图、哈希、选框、模型结果、仪器/场景快照及时间分别留存。

摄像头码尚未用于领用登记。本版实验员由页面填写，设备标识来自取流协议或上传声明，不具备用户认证/硬件认证。两张仪器码沿用已有唯一 ID，未签名，不具有防复制能力。

## 接收端协议

参考用户提供的《下游_全路RGB主码流_Windows_Python开发与接口协议.md》（2026-08-11 / f806ba9）。

- 设备发现：`GET /api/status`，以 `sender_id` 和 `camera_id` 为身份，选择 `online && media_live` 的目标相机。
- 主码流：`GET /api/preview/rgb-h264-frames?sender_id=...&camera_id=...&quality=main&metadata=global`。
- 校验 `X-GWV3-Rgb-Stream: main`；解析 GWHP v1/v2、扩展头及长度边界；不把 HTTP chunk 当视频帧。
- 单目标相机、两包上限队列、丢包后等待 KEY|CONFIG 并重建解码器；只保留最新解码图，超过一秒拒绝当作新照片。
- 保存帧 PTS、sequence、sender 时间、global 时间与 clock_sync_valid；不额外加 offset。
- 本项目 使用 PyAV CPU 单路解码，**不是协议中要求的生产级全路 NVDEC 验收**。软件解码和 PaddleOCR 都不占用 NAS YOLO 的 GPU。
- 不直连 sender 内部媒体端口，不连接 Depth，不修改接收端连接容量。

当前 receiver：`http://192.168.1.196:8080`，目标：`lubancat-52d2ef0c_cam01`。2026-09-11 已实测主码流连接及真实取图，分辨率 1280×800。拍摄证据与时间戳见 `Verification/ActualNeckCapture.json`。当前画面未出现仪器二维码或可读面板，尚不能证明真实仪器绑定及面板识别准确率。

## 运行

独立 Python 环境：`.venv`，已安装 CPU `paddlepaddle==3.2.0`、`paddleocr==3.2.0`、`av==16.1.0`。完整依赖见 Requirements.lock.txt。

用户服务：`field-recognition-demo.service`。当前只监听本机 8188，CPU 上限 2 核、Nice=10，不对公网开放。

```bash
systemctl --user start field-recognition-demo.service
systemctl --user stop field-recognition-demo.service
```

在该服务环境配置中更新 `FIELD_RECEIVER_URL` 后，仅重启此项目服务。不要重启 `visioncortex-analysis.service`。

主要环境变量：

- `FIELD_RECEIVER_URL`：receiver 的 HTTP 地址。
- `FIELD_CAMERA_ID`：目标 camera key，从 `/api/status` 精确匹配后取得稳定身份。
- `FIELD_DEMO_DATA`：默认项目下 Data；测试使用独立目录。
- `FIELD_CAMERA_SNAPSHOT_URL`：无 GWHP receiver 配置时可选的 JPEG/PNG 接口，不与 GWHP 混用。

## 独立接口

- `GET /api/state`：设备登记、绑定、OCR 和取流状态。
- `PUT /api/instruments/{id}`：登记仪器和所属场景。
- `POST /api/scans`：上传照片，解码二维码并保存证据。
- `POST /api/camera/capture`：读取所选挂脖设备最新主码流帧并扫码。
- `GET /api/camera/preview.mjpg`：不落盘的实时 MJPEG 预览，最多 15 fps。
- `POST /api/camera/scan-sessions`：传 `{"operator":"实验员"}`，开始连续扫码。
- `GET /api/camera/scan-sessions/{session_id}`：读取结果并续期；建议每 0.5 秒读取。
- `DELETE /api/camera/scan-sessions/{session_id}`：停止扫码，已有绑定保留。
- `POST /api/bindings`：确认使用关系；同相机、仪器和实验员重复提交返回原绑定。同相机当天内可直接更换操作人，保留绑定 ID、开始时间、有效期和二维码证据，追加 `operator_history`；其他仪器可独立关联，另一相机占用的仪器仍走明确交接流程。
- `POST /api/bindings/{id}/end`：结束使用。
- `POST /api/ocr`：提交 binding_id、capture_id 和可选 crop=[x,y,w,h]，异步返回 job_id。
- `GET /api/jobs/{id}`：读取状态和 OCR 原文、坐标、模型分数。
- `GET /api/export`：导出登记、绑定和 OCR 结果；原图片以引用提供。

相机身份不同、照片中的仪器码与当前绑定冲突、绑定已结束或面板选框越界时拒绝识别请求。面板照片若没有二维码，其仪器关联依据为操作人选择的当前绑定，而非模型已证明同一仪器。

## 验证边界

- tests：真实二维码解码、绑定隔离、历史快照、错误输入、异步派发、协议拆包、真实 H.264 CPU 解码及过期帧拒绝。
- Verification/ActualOcr.json：真实 PaddleOCR 模型调用；输入为带明显标记的合成测试面板，不是实际设备拍摄，不能作为仪器准确率结论。
- Verification/BrowserCheck.json：独立测试数据下的浏览器扫码、登记、绑定、框选和 OCR 流程；与现场运行数据隔离。
- 挂脖设备主码流连接与真实取图：PROVEN，见 Verification/ActualNeckCapture.json。
- 真实仪器扫码绑定及面板准确率、生产级多路 GPU 吞吐：NOT_PROVEN。

PaddleOCR 安装与调用依据：https://www.paddleocr.ai/main/en/quick_start.html 。

## Agent 小工具

已提供九个共用现有实现的 HTTP JSON 工具，工具发现入口为 `/api/tools`。接入说明见 [AgentTools.md](../AgentTools.md)，标准库 Python 适配器见 [agent_client.py](../agent_client.py)。`Verification/AgentTools.json` 记录工具接口真实取图验证和测试边界。

## 场景扫码接入（2026-09-11）

场景 01 已登记为湿实验实验台，称量仪器 A/B 均属于该场景。先用挂脖设备拍摄场景二维码，点击“确认进入场景”；再用同一设备拍摄仪器码，填写实验员并绑定，之后拍面板提交 OCR。同场景重复确认幂等；当前有仪器绑定时拒绝切换场景，须先主动结束或等待设备采集服务重启。其他相机的场景记录不可借用，仪器场景不一致拒绝绑定。每次 OCR 无需重复扫码，但必须保留有效的仪器绑定和场景记录。

页面和 Agent 共用 `/api/scene/enter` 与 `enter_scene` 工具；参数为 scan_id、scene_id。状态包含 scenes、scene_visits；扫码包含 scene_matches；绑定及 OCR 回执保留 scene_visit_id、场景快照和原图引用。原二维码不变，仍是未签名二维码。场景所属关系由登记提供，扫码不能证明真实地理位置或硬件身份。

23 项测试通过，包括真实二维码解码、相机隔离、场景切换、重复进入、仪器绑定及 OCR 前置验证。浏览器使用样张相机 验证场景样张到仪器绑定，验证绑定已结束。真实挂脖设备在现场拍摄贴纸的完整验证尚待用户操作。

## 2026-09-11 实物扫码排查

扫码增加 ZXing-C++ 2.3.0 补充解码、灰度放大与局部对比度重试，保留原图、真实解码内容与原图坐标；扫描记录新增 qr_diagnostics。只接受校验通过的码内容，不根据标签文字或已登记设备猜测身份。依据：https://github.com/zxing-cpp/zxing-cpp/tree/master/wrappers/python 。

25项测试通过，但本次12张挂脖实拍仍均未解码成功，见 Verification/QrDecoderActualImages.json；不能称实物问题已修复。近处码块模糊、部分照片存在反光。请先固定平整标签，调整距离至黑白格边缘清楚，再重拍；用手机拍清晰照片上传可区分打印问题与挂脖镜头成像问题。


## 连续视频扫码与采集服务会话（2026-09-11）

`live_scan.py` 共用现有二维码、场景校验、原图存证和绑定实现。扫码使用原始分辨率的新鲜帧：优先 ZXing 原图快速解码，每秒一次完整补充解码；只处理新的已解码帧，CPU 忙时跳过中间帧以免积压，不承诺逐一处理全部 30 fps 源帧。只要实际处理的一帧成功解码出唯一合规仪器码，即可绑定，不要求多帧一致。预览独立缩放至宽度最多 960 像素、最多 15 fps；断流超过一秒显示等待画面。未命中帧不存图，命中帧保留原始像素、哈希、帧时间与真实解码结果。

每台挂脖相机只有一个连续扫码会话。绑定成功或多码需要选择时结束自动扫码；页面停止、离开或 20 秒不读取状态也会停止。终止返回后，进行中的解码不能补写绑定。已有绑定保存在 SQLite，刷新页面或重启本地识别服务 不会因为内存状态丢失而重复绑定。主动结束仍作为操作人明确发起的退出入口保留。

设备服务边界来自每秒读取 receiver `/api/status` 的目标相机 `status_live`、`media_live` 和 `rgb_ingress_session_id`：两种 live 都为 false，或 RGB ingress 会话改变，结束该相机旧场景与仪器绑定；重新在线设置新的扫码时间界限，旧照片不能再用于进入场景或绑定。`recording_session_id` 不参与判断。接收端 HTTP 失败、本服务的 GWHP 读取连接重连不直接结束绑定。状态保存在 `camera_service_state`，结束原因留在历史记录中。

**边界：** receiver 的 ingress 会话不是设备进程的启动 UUID；设备重连或 receiver 重启也可能改变它。当前实现按可观测的采集服务可用性/会话边界要求重新扫码，不能严格证明“只在 sender 进程重启时变化”。若需区分全部网络重连与设备服务重启，仍需上游提供稳定的服务运行实例 ID。此项目未修改上游服务。

验证：36 项自动测试（含样张真实解码、单帧命中、幂等绑定、取消竞态、多码暂停、会话续期、设备离线/重新上线与跨相机隔离）通过。真实挂脖视频预览已检查，回执见 `Verification/ContinuousScan20260911/Receipt.json`；隔离浏览器验证见同目录 `BrowserReceipt.json`。这次未启动真实 PaddleOCR、未创建正式测试绑定、未停止设备采集服务。

- PROVEN：隔离环境中的连续扫码及绑定规则；实际挂脖视频预览。
- PARTIAL_EVIDENCE：接收端采集会话字段已读到，重启失效逻辑已通过模拟测试。
- NOT_PROVEN：现场仪器标签连续扫码完整绑定、实际设备采集服务停启闭环、真实面板准确率。


## 绑定后 OCR 常驻（2026-09-11）

手动绑定、连续扫码自动绑定和 Agent 的 bind_instrument 共用同一实现：绑定事务成功提交后，异步将模型加载任务放入单线程 CPU OCR 队列，绑定接口直接返回。加载和识别共用同一个工作线程及模型锁；并发或重复绑定不会创建多个模型实例，也不阻塞二维码绑定。初始化只加载模型，不会自动读取视频面板。面板预测由该相机新语音照片、网页识别按钮或 Agent 的 read_panel/read_saved_panel 请求触发。

PaddleOCR 加载成功后在本地识别服务进程内持续保留。完成一次识别、主动结束绑定、相机服务离线或重新绑定都不会卸载模型。只有停止/重启本地 `field-recognition-demo.service` 才释放该进程中的模型；如果服务启动时数据库仍有有效绑定，会自动再次后台加载，无需重复绑定。加载失败会保留已有绑定并展示错误；后续绑定请求或面板识别可重试加载。

`GET /api/state` 的 `ocr` 提供 `status`（not_loaded / queued / loading / ready / error）、`resident`、`loaded_at`、`load_count` 和 `load_seconds`。这些字段描述模型进程状态，不表示已识别到面板，也不是准确率。

验证：40 项自动测试通过，覆盖后台加载不阻塞绑定、并发初始化去重、预测复用、结束绑定后保留、失败重试、已有绑定随本地服务启动恢复预加载。测试使用模型替身。真实 CPU PaddleOCR 加载与复用单独在临时数据中检查，回执为 `Verification/OcrResident20260911/Receipt.json`；不修改正式绑定，也不运行面板预测。


## NAS 语音照片监控与读数兜底（2026-09-11）

当前配置只读 `/mnt/realityloop-nas/voice_photos/<camera_id>/YYYY-MM-DD/HH-MM-SS/YYYYMMDD_HHMMSS[_序号].jpg`。这是本次读取 NAS 目录后确认的语音拍照布局；同一次语音拍照目录可能包含多张照片，按文件分别去重处理。不监控相机视频分片目录，不重新取视频帧，不修改、移动或删除 NAS 文件。

`photo_watch.py` 每 2 秒检查当前有效绑定相机的目录；没有绑定时不读取 NAS。只考虑文件名拍摄时间在本次绑定开始之后的照片，连续两次观察到大小及修改时间稳定、间隔至少 1 秒后提交。每轮最多提交一张，待处理队列最多四个。读取失败/文件变化、相机不符、时间过旧、二维码与绑定冲突均不输出读数。绑定结束、场景变更或采集服务会话导致绑定失效后，不再接收该绑定的新照片，进行中的结果也会检查绑定是否仍有效。

照片按源相机、源照片编号和内容 SHA-256 导入一次；同一绑定、同一照片、同一选框返回原任务，包括已完成的任务。文件监控回执保存到 SQLite，重启本地服务不会重复处理已经记录的文件。原始 NAS 路径、源图哈希、文件名推导的拍摄时间和本地接收时间分别保留；文件名时间采用 `FIELD_SAVED_PHOTO_TIMEZONE`，默认 Asia/Shanghai，精度只有秒，不等于设备硬件时间认证。原图读取后在本项目保存用于识别和回执的副本，NAS 原件不变。未框选时识别整张照片，尚未自动定位或跟踪物理面板，照片需对准当前绑定仪器。

`panel_readout.py` 将请求调度与 CPU 模型线程分开。每张照片先提交给常驻 PaddleOCR：出现完整数字行候选就立即返回，低模型分数本身不触发兜底。请求实际开始后 5 秒仍没有数字（包括模型仍在执行）时，最多调用一次阿里云 `qwen3.8-max`；同一绑定两次付费尝试至少间隔 30 秒，并持久化间隔。没有自动 HTTP 重试。若多张照片接连失败，间隔内的后续照片只返回本地结果和后台 skipped 回执。云端仍无法看清时保留无读数，不补数字；本地模型晚返回的原始结果另留审计，不改写已交付读数。

默认模型依据阿里云当日模型列表：<https://www.alibabacloud.com/help/zh/model-studio/models>。视觉接口：<https://help.aliyun.com/zh/model-studio/vision>。密钥仅从环境读取，当前保存在忽略提交的 `.env`（0600），由用户服务的 EnvironmentFile 加载。只向阿里云发送本次面板选框（未选框则整张照片）和固定读数提示，不发送实验员、绑定记录或 NAS 路径。原始本地 OCR、视觉请求回执和来源保留在 JSON，网页统一显示读数，不单列云端结果或伪造模型分数/文字坐标。

配置见 `.env.example`：`FIELD_SAVED_PHOTO_ROOT`、`FIELD_SAVED_PHOTO_WATCH_ENABLED`、`DASHSCOPE_API_KEY`、`FIELD_ALIYUN_FALLBACK_ENABLED`、`FIELD_ALIYUN_MODEL`、`FIELD_ALIYUN_NO_DIGITS_SECONDS`。手动启动需显式设置这些环境变量；程序不自动加载 `.env`。

新增 `POST /api/ocr/photo-result` / Agent `read_saved_panel` 可传 `binding_id` 和确切 `image_path`，也可用 `photo` 对象传原图 base64 和拍照回执；不允许两个来源同时传。路径必须属于已配置的语音拍照目录和绑定相机，不支持任意文件、目录、视频或自动挑选最新照片。`GET /api/state` 返回 `photo_watch` 状态，`jobs` 返回统一读数与后端回执。详见 AgentTools.md。

验证材料：`Verification/VoicePhotoWatch20260911/NasSourceReceipt.json` 是一张实际 NAS 语音照片的只读解析；`Verification/AliyunFallback20260911/SyntheticReceipt.json` 是阿里云真实视觉调用的合成面板测试。`Verification/VoicePhotoWatch20260911/SyntheticPipelineReceipt.json` 进一步记录真实 CPU OCR→5.002 秒超时→真实阿里云对合成空白面板返回 unreadable 的完整链路，输入目录是本机临时目录。66 项自动测试通过；自动测试使用临时照片目录、临时数据库和模型/HTTP 替身，不访问真实 NAS、相机或付费模型。NAS 路径与单张照片读取为 PROVEN；真实语音指令→新照片→当前物理仪器读数闭环仍是 NOT_PROVEN，不能以合成面板和服务响应代替现场准确率验收。


## 2026-09-14：未绑定语音照片与逐阶段时延

本次启用配置相机独立的语音照片监控；无绑定也执行 OCR 与留存，原始照片和历史绑定不改写。已补处理 11:52:48–11:56:36 的 24 张真实语音照片，均调用本机 GPU，12 张有完整数字候选、12 张没有；数字候选尚未证明是正确面板读数。模型推理中位数 25.955 ms，范围 16.79–450.77 ms。24 张均为补处理，不用其延迟推断新照片实时性。

本机核对源照片 SHA-256、NAS 回执文件哈希及所有原件/规范化图/面板图哈希一致；归档队列已清空，167 个历史及新版本回执已归档。所有这批任务的仪器、场景、binding_id 为空，保留既有实验员登记依据。材料：`Verification/UnboundPhoto20260914/Receipt.json`。

实际阿里云请求返回 HTTP 400、Arrearage，账户欠费或余额状态限制；没有真实模型输出。增加安全的错误码分类和界面提示，不写入可能包含敏感内容的服务错误原文。24 张照片的增强二维码解码仍均未成功；放大、分块不能恢复缺失的原始细节，未创建假绑定。

110 项测试通过，使用临时目录与模型/HTTP 替身；Python 编译、JavaScript 语法与差异格式检查通过。新增覆盖未绑定照片、自动关联冲突、绑定结束后保留照片任务、重传去重、阶段时间、时区排序、归档确认及 Arrearage 脱敏。近实时新照片闭环、二维码实物绑定、读数逐位准确率和账户恢复后的兜底仍需现场验收。字段和执行规则见 [识别时延与未绑定照片](识别时延与未绑定照片.md)。

## 2026-09-14：独立等待与读数历史查询

照片请求调度从一个线程改为四个，最多四个进行中任务。单张照片的五秒等待或视觉请求不再阻塞另一张已获得数字候选的照片；GPU 模型仍由一个线程串行调用。付费视觉请求增加全局互斥，同一绑定或未绑定相机仍遵守原有三十秒间隔。视觉服务正在处理其他照片时，保留本地结果及 `skipped/busy` 原因，不排队追加付费调用，也不消耗新的间隔。

操作记录新增“全部读数”游标翻页、照片预览、任务回执链接、逐阶段时延和每条记录最新版本的归档状态。保留“最近操作”视图；“导出状态 JSON”仍导出当前状态快照。历史补处理明确标记，不计入新照片实时性能。

验证使用项目 `.venv` 和临时数据，114 项测试通过。新增用可控模型/HTTP 替身验证前一任务等待视觉响应时后一任务能够完成、GPU 调用峰值为一、任务上限为四、视觉互斥及翻页期间新增记录不会造成后续页重复。Python 编译、JavaScript 语法、文档引用及差异格式检查通过。

本机部署前备份 SQLite，队列空闲时重启本项目服务。只读 API 和浏览器实测“全部读数”为 20 条及 4 条两页，共 24 个唯一任务，照片、阶段耗时和归档状态均可查看；部署前后仍为 24 个任务、53 条扫码/照片记录、167 个归档版本，待同步为零。GPU 模型重新加载后常驻，未重跑已有照片或主动发起新的付费请求。部署核对材料为 `Verification/ReadoutOptimization20260914/Receipt.json`。

- PROVEN：受控条件下的调度隔离、并发边界及分页规则；实际服务的历史查询和已有归档状态展示。
- PARTIAL_EVIDENCE：GPU 常驻状态已确认，未用新的现场照片测量优化后的吞吐。
- NOT_PROVEN：新语音照片从 NAS 写入到识别结果的实际端到端提速，以及真实仪器读数准确率。阿里云账户限制未解除，不能宣称视觉兜底恢复。

## 2026-09-14：NAS 全量索引与每日完整性巡检

NAS 索引升级为全量摘要，按实验员、仪器、北京时间事件日期、相机和类型组合筛选；支持最新业务快照与全部历史版本、分页及筛选 JSON 导出。旧字段与人员原值保留在结构化数据中，页面沿用既有历史样张名称展示规则。增加本机只读归档文件路由，拒绝路径越界和指向归档外的符号链接。

独立后台巡检默认每 86400 秒执行一次，报告和下次计划由本机 SQLite 持久化；只读校验业务回执与其引用的图片，异常保留具体文件、版本和原因。报告在本机保存后同步到 NAS `Integrity/Reports/`，`Integrity/Latest.json` 指向最近结果。报告不改写业务凭证，NAS 中断时延迟发布并保留重试。

122 项自动测试通过。新增覆盖 206 个版本中的早期记录查询、最新快照再筛选、北京时间日期边界、未知身份、HTML 内容转义、越界拒绝、缺失文件、照片/回执损坏、重复对象只校验一次、失联报告补传、中断状态和重启后巡检续期。自动测试全部使用临时文件系统与模型替身；Python 编译、两份 JavaScript 语法与差异格式检查通过。

本机 2026-09-14 13:18:35 完成首次实际巡检：167/167 份回执、101 个图片对象通过校验，读取约 80.4 MB，异常为零。全量索引包含 81 条业务记录、167 个版本。浏览器按徐荣炜、2026-09-14 和面板读数组合筛选得到 24 条最新记录，切换全部版本得到 86 个版本，已核对翻页。重启本项目服务后仍为同一份巡检报告，下次计划保持 2026-09-15 13:18:35（Asia/Shanghai）。

部署前后 268 个既有回执及图片文件 SHA-256 不变，业务仍为 24 个任务、53 条扫码/照片记录、167 个归档版本；未主动提交新 OCR 或视觉调用。验证材料：`Verification/ArchiveIndexIntegrity20260914/Receipt.json`，部署前 SQLite 副本位于同一忽略目录。

- PROVEN：本轮 NAS 文件一致性、全量索引查询、实际报告发布和服务重启后保留巡检计划。
- PARTIAL_EVIDENCE：跨日调度及 NAS 失联恢复通过受控测试，本轮未等待完整 24 小时或断开实际 NAS。
- NOT_PROVEN：权限级防篡改、完整运行数据库灾难恢复、真实仪器读数准确率。全量索引当前覆盖本机数据库已确认归档的版本；其他数据库或未登记文件不在本次巡检范围。

## 2026-09-15：按日期、相机与测量整理归档

本机完成旧归档迁移：804 份不可覆盖回执、466 张图片、1,270 个原文件引用逐一读回，SHA-256 一致，完整性检查 0 项异常。生成 186 个事件目录，其中 139 个二维码/绑定及读数事件用于日常查询，47 个历史未分类材料放在隐藏维护区。可见事件含 17 个二维码/绑定事件、96 次独立语音照片采集、25 次视频证据采集、1 次其他照片采集。旧照片没有显式连拍 ID，未按相近时间擅自合并；新接口显式连拍与确认共用一个测量目录。

迁移前数据库与哈希清单保存在本机 `Verification/ArchiveMigration20260915-155629/`。历史业务文档、人员、时间和回执字节不改写；`Result.json` 是采用当前规则生成的可更新查阅结果。标准仪器格式为 `instrument-measurement/1`，导出规则为 `instrument-measurement/2`，照片草稿规则为 `photo-measurement/2`。新结构见 [NAS 文件夹与留存指南](NAS文件夹与留存指南.md)。

- PROVEN：NAS 实际迁移、全部旧引用的文件一致性、296 个静态导航链接无缺失；浏览器能从日期报告直接打开原图；旧回执、Records/Jobs/Bindings、Browse 入口兼容。相机推流、GPU OCR 常驻、照片监控及归档进程恢复，待同步队列为 0。
- PROVEN（代码验证）：回归覆盖显式连拍的一次测量、冲突校正、确认与重传不重复归档、单图 A/B 分字段、单位冲突不从登记默认回填、同步有效的视频帧时间、迁移中断恢复、目标冲突保护和手写文件保留。
- PARTIAL_EVIDENCE：现存具备面板定位和绑定证据的标准化历史结果为搅拌器；天平格式及双设备拆分通过受控回归验证，不补造历史缺失的天平归属或读数。原数据没有可靠单位、拍摄时间或范围时保持 null。
- NOT_PROVEN：本轮没有发起新的现场拍照来验证新采集的端到端读数；NAS 中断恢复由故障注入验证，没有人为切断现场 NAS。


## 2026-09-15：最新照片展示与红色数码屏 1/7 混淆修复

- `PROVEN`：289 项临时数据库回归检查通过，Python 编译、两份工作台 JavaScript 语法检查通过。覆盖新照片无读数覆盖旧结果、迟到照片时间排序、读取失败重启恢复及错误原因保留、温度／转速定位框轻微重叠、OFF 不转换为数字或触发云端兜底。
- `PROVEN`：用户指出的照片拍于 13:21:00、任务于 14:25:11 开始；原始回执中读取前等待约 3849.45 秒、OCR 18.34 毫秒、写入至结果 3855.65 秒。页面现在分开展示拍摄与处理时间、读取前等待与任务耗时；历史总耗时不被改小，未记录的等待原因不补造。
- `PARTIAL_EVIDENCE`：对同一 NAS 照片的本机副本，复用当时真实检测框并重新执行 GPU OCR。红色数码屏分段笔画校验将原 OCR 的 12 纠正为 72，右侧为 OFF；恢复温度／转速字段，耗时 0.0215 秒。原始照片、旧任务和历史回执未修改。证据在本机 `Verification/ReadoutIssue20260915/ReplayResult.json`。
- `NOT_PROVEN`：该复核未重新执行面板检测，也不是新拍摄的端到端现场验收或总体准确率结论。分段校验不推断小数点、负号或单位，其他数字冲突继续进入兜底／校正流程。

## 2026-09-16 处理链路补齐

- NAS 最近照片发现与历史补查分开，文件 I/O 使用有时间上限的独立进程；每文件持久化退避重试，新照片三次/补录一次的调度保证两类都获得机会。
- 自动监控读取 `PhotoReceipt.json`，将明确连拍编号、照片编号、拍摄时间、哈希和实验上下文交给原测量模块。旧 JPG 缺少明确连拍回执时仍独立处理。
- 新增完成结果的显式重识别与版本查询；旧结果和原绑定快照不变。正式测量再识别进入修订草稿，再确认才产生正式新版本；同测量保持一个 NAS 目录。
- 量程与显示小数位校验统一用于规范字段、草稿与前端。登记精度从实际设备资料填写，不采用仪器类型的默认精度猜测小数点。
- 本轮没有训练模型、修改源采集目录或修改 VisionCortex 服务。测试与现场证据范围见本轮交付说明。

## 2026-09-16：开机模型恢复与每日扫码

- 本机四项服务在 09:39:37 自动启动，target 已启用、用户 Linger=yes；启动故障发生在 OCR 模型加载而非 systemd。PaddleX 先检查模型站点，即使完整缓存已存在仍报无可用模型源。改为显式加载完整本机目录，错误重试从 5 秒退避至最多 300 秒。
- 10:58 仅重启本项目 OCR 后，GPU OCR、面板定位均 resident=true，视频推理计数从 0 增至 87。整组服务随后于 11:07 更新，均 active/running。未重启其他项目。
- 新绑定按 Asia/Shanghai 当天零点到期；仪器、场景、未完成交接均要求当天新扫码。旧码不能换成新的绑定，短暂断流不提前结束当天关系。后台独立执行到期，跨日停机后恢复会补记准确到期时间。原照片和已完成任务不改写。
- 实际昨天的两条仪器关系及一条场景关系已以 daily_qr_expired 结束；当前 waiting_binding 是等待当天二维码，不是模型故障。
- 原任务 e8740410-a0ad-4451-81c0-398723ff1cdd 创建识别版本 2（9e556445-9067-4c74-9be5-91d3e25b76cf），同一原图得到温度字段 72、转速显示 OFF；OFF 不变成 0，未确认的单位仍为 null。原任务的 12 原文保留；新版本归档使用同一照片来源。
- PROVEN：临时数据全量 307 项通过，追加失败版本恢复用例通过（相关 9 项通过）；Python 编译、JavaScript 语法及 diff 检查通过。真实本地 GPU 对上述指定原照片的新版本完成并归档；详细回执位于本机 Verification/ReadoutImprovements20260916/，不进入 Git。
- PARTIAL_EVIDENCE：修复后的整机断电重启尚未另做一次；系统原有自启日志、已启用配置及服务重启加载已核实。新扫码后的现场视频读数仍需新的当天画面。此单张照片不代表所有仪器的识别准确率。
- NAS 连拍回执适配已经通过临时目录完整导入与重复传输检查；现有采集文件未提供 PhotoReceipt.json，未将同秒照片强行合并，也未修改上游采集服务。

## 2026-09-18：现场反馈修复与代码回归

本次验证基于 `main`、HEAD `ee0dbf31710a5505b0132bb2a1f564da9fd9c6b0` 加本地未提交改动，不能将 HEAD 单独视为包含修复的发布版本。

- `PROVEN`（受控代码验证）：完整临时数据测试 **372 passed**；项目 `.venv` Python 编译、`node --check static/app.js` 和 `git diff --check` 通过。测试不证明真实仪器准确率或部署完成。
- 当天同相机、同仪器可直接更新操作人，保留绑定 ID、开始时间、有效期及二维码证据，追加人员变更历史并归档旧版本；迟到照片按拍摄时间恢复操作人。相机支持登记多名成员，另选一名当前使用人用于后台采集。接口见 [托管运行说明](ProductionRuntime.md)。前端已移除必须先结束绑定才能换人的提示。
- 搅拌器登记迁移提供温度 `°C`、转速 `rpm` 字段；面板检测与数字恢复覆盖新增受控回归。明确识别的 OFF 导出为 `value=null`、`display_state="OFF"`，契约见 [测量 Schema](../schemas/InstrumentMeasurement.schema.json)。历史缺失单位和原始回执不改写。
- `PARTIAL_EVIDENCE`：既有本地复核材料 `Verification/Badcase20260917_1913_1920/Summary.json` 记录指定 19:20:47 原图的新识别版本得到温度 63、转速 1140，旧版本与源图哈希保留；该次复核结果单位仍为 null。历史任务白名单含搅拌器 UUID `e9434a0a-3319-414a-b988-4cc6884edce4`，不能把该次 `boxes=0` 归因为未绑定。此材料不替代当前完整版本的现场复测。
- `PROVEN`（受控代码验证）：面板裁剪、绑定过滤及定位失败时不发送整图到视觉兜底的回归通过。`NOT_PROVEN`：用户报告的“天平 + 笔记本 + 墙上设备”两张原图的当前版本结果，以及外部 VLM 报告中 clear/medium 冲突的消除；不能据此宣布多屏幕鲁棒性问题已关闭。
- 已补充结果生成、不可变回执发布、派生视图发布的阶段时间；无法还原的历史时间保持 null。写入端视图发布时间与 NAS 客户端首次成功读取时间有不同含义，见 [时延说明](识别时延与未绑定照片.md)。`NOT_PROVEN`：修复后十组连续注入的端到端时延及稳定上限；产品／验收阈值仍待确认。
