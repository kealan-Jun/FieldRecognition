...
function renderOcr(job){
  ...
  target.innerHTML=readoutPhoto(job)+`<p class="caption">${esc(job.instrument?.name||(job.association_status==='localized_panels'?'A / B 面板分别定位':job.instrument_candidates?.length?'多仪器候选 · 归属待确认':'未绑定仪器'))} · ${esc(stamp(job.external_photo?.captured_at||job.video_observation?.observed_at||job.submitted_at))}<br>识别结果，尚未人工核对</p>${readoutLatency(job)}${lines.length?(job.readings?.length?job.readings:lines).map(line=>`<div class="ocr-line">${line.instrument?`<small>${esc(line.instrument.name)}</small>`:""}<strong>${esc(line.text)}</strong>${line.panel_image_url?`<a href="${esc(line.panel_image_url)}" target="_blank" rel="noopener">本面板照片 ↗</a>`:""}${line.association_basis==='ambiguous'?'<small>多个仪器候选，读数归属待确认</small>':''}${line.quality_issue?'<small>小数点可能缺失，请核对原图</small>':''}</div>`).join(''):'<p>这次照片未读出数字，请调整角度、清晰度或选框后重新拍摄。</p>'}${readoutIssue(job)}<p><a href="${job.crop_image_url}" target="_blank" rel="noopener">查看面板照片 ↗</a> · <a href="/api/jobs/${job.job_id}" target="_blank" rel="noopener">结果 JSON ↗</a></p>`;
}
