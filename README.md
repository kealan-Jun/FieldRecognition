...
> 2026-09-20 本地修复：多人名单与实际使用人分离，交接保存独立使用时段；场景支持多相机共享。绑定快照、正式字段过滤、检测分阶段诊断及发布一致性已补回归，真实图片与现场 NAS 验收仍待完成。见 [修复与验证说明](docs/FieldRepair20260920.md) 和 [数据库迁移](docs/BindingMigration.md)。

> 2026-09-20 运行恢复：补充 OCR 接口依赖恢复定时器、启动前迁移演练与备份、停写后升级入口，以及保留旧回执的失败任务新版本。安装、维护暂停和验证边界见 [运行可靠性说明](docs/RuntimeReliability.md)。

> 2026-09-17：工作台可接独立 GPU OCR 服务；照片在服务离线或调度切换时持久化等待，后台异常退出自动恢复。配置与运行边界见 [工作台推理连接与故障恢复](docs/工作台推理连接与恢复.md)。
...
视频扫码持续寻找有效二维码，**实际处理的一帧解出已登记的仪器码，即可逐项完成绑定**。同会话内同一码只绑定一次，持续扫描仅用于发现新码；当天断流或 Receiver 接入会话变化保留绑定；北京时间跨日、明确设备启动标识变化或主动结束后需重新扫码。启用照片监控即在本地后台加载 PaddleOCR，后续照片复用常驻模型；单张读数任务运行 5 秒仍无完整数字候选时，按配置调用阿里云视觉模型兜底。
...
| 🔗 **会话内持续绑定** | 同一相机可同时绑定多台已登记设备；同一二维码在活动期只能归属一台设备，重复提交沿用原绑定，设备采集服务会话失效后结束旧关系。 |
| 🗂️ **设备区分与分类** | 每台二维码设备保留唯一身份凭证、设备分类和面板类型；`/api/devices` 提供按分类查询及当前相机归属。 |
...
1. **登记并扫码。** 填写仪器信息和设备分类，由使用者登记实验员，点击“保存并开启自动运行”，对准仪器码。已登记的仪器码成功解码后逐个自动绑定，不要求先扫场景码；同一相机可同时关联多个二维码设备，但一个二维码在活跃绑定期间不能被另一台设备重复占用。明确结束绑定、跨日失效或确认设备启动标识变化后，才允许把同一二维码重新分配给替换设备，历史绑定和扫码证据仍保留。关闭网页仍会继续；手动连续扫码入口也保留。设备目录可通过 `/api/devices` 查看。
