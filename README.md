...
视频扫码持续寻找有效二维码，**实际处理的一帧解出唯一且已登记的仪器码，即可完成绑定**。有效绑定期间无需为每次读数重复扫码；设备采集服务离线或采集会话变化后，需要重新绑定。启用照片监控即在本地后台加载 PaddleOCR，后续照片复用常驻模型；单张读数任务运行 5 秒仍无完整数字候选时，按配置调用阿里云视觉模型兜底。
