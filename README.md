...
视频扫码持续寻找有效二维码，**实际处理的一帧解出唯一且已登记的仪器码，即可完成绑定**。有效绑定期间无需为每次读数重复扫码；设备采集服务离线或采集会话变化后，需要重新绑定。绑定成功即在本地后台加载 PaddleOCR，后续照片复用常驻模型；单张读数任务运行 5 秒仍无完整数字候选时，按配置调用阿里云视觉模型兜底。
...
| 🔢 **常驻本地 OCR** | 绑定后后台加载 GPU PaddleOCR；对具体照片执行识别，支持手动框选面板，后续任务复用模型。 |
...
1. **登记并扫码。** 填写仪器信息，由使用者登记实验员，点击“保存并开启自动运行”，对准仪器码。唯一且已登记的仪器码成功解码后自动绑定，不要求先扫场景码；多个候选需要手动选择。关闭网页仍会继续；手动连续扫码入口也保留。
